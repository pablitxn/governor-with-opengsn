...
export function printSampleEnvironment (defaultDevAddress: string, chainId: number): void {
  ...
  console.log('No configuration found:', '\nPlease add the following to', `"${deploymentConfigFile}"`, ':\n\nmodule.exports = ',
...
export async function printRelayInfo (hre: HardhatRuntimeEnvironment): Promise<void> {
  ...
  console.log('Example for Relayer config JSON file:')
  ...
  console.log('Relayer register:')
  console.log(`gsn relayer-register -m $SECRET --network ${network.url} --relayUrl https://${hre.hardhatArguments.network}.v3.opengsn.org/v3 --token ${stakingTokenAddress} --stake ${stakingTokenValue} --wrap`)
}
